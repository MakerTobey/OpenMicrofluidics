...
class controlGUI:
    ...
    def oneCycle(self):
        ...
        if self.cycleCount < self.cFinal:
            self.sync_move(int((3500*self.vFinal)-1350))
            print "moving", str((3500*self.vFinal)-1350)
            self.cycleCountLabel.config(text=self.cycleCount)
            root.after(1000, self.update_clock)
            root.after(1000, self.update_timer)
            self.job = root.after(100,self.doCycle)
            ...
            print('Jamin fucked up. Not sure what happened, but RESET!')
            self.stopAll
            GPIO.cleanup(22)
